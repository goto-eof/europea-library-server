...
- During my tests (in debug mode) I noticed that the job, in order to index and extract metadata from 8.850 files in a
  ...
  same notebook I ran
  the multi-thread job and the result is the following: about 13 minutes to index 8.850 files. I also run the
  job on a set of about 110.000 ebook (~25.000 epub, ~25.000 pdf, ~60.000 other files), on i7-10750H (6 core, 2.60GHz)
  equipped with SSD, and it took 1 hour and 15 minutes to finish
  the job. Some steps were skipped (like FSI/FMI deleter) because I started the job on an empty database.

![job-completion](/images/job_time.png)
